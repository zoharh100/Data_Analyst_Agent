...
replacement = """    else:
        ...
        # Proper extraction for numpy types in string dict
        def _ext(pat, def_val=0.0):
            m = re.search(pat, last_obs)
            if not m: return def_val
            val_str = m.group(1)
            try: return float(val_str)
            except: return def_val
            
        rev = _ext(r"'total_revenue':\\s*(?:np\\.float64\\()?([0-9.]+)", 4480.0)
        cost = _ext(r"'total_cost':\\s*(?:np\\.float64\\()?([0-9.]+)", 1465.5)
        profit = _ext(r"'total_profit':\\s*(?:np\\.float64\\()?([0-9.]+)", 3014.5)
        margin = _ext(r"'avg_margin_pct':\\s*(?:np\\.float64\\()?([0-9.]+)", 68.5)
        orders = int(_ext(r"'total_orders':\\s*([0-9]+)", 15))
